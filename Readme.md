...
To get things up and running on a different machine, just follow the steps above again.

To run again on my zenbook, should only need to:

// Get Redis running with docker:
`docker run -d --name redis -p 6379:6379 -p 8001:8001 redis/redis-stack:latest`

// Restore the python env: (note - recreate if on a new machine)
`venv\Scripts\activate`
